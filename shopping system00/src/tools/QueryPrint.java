...
public final class QueryPrint {
    ...
    public ArrayList<Goods> queryGoodsKey(int gId,String gName)
    {
        ...
        try
        {
            ...
            pstmt.setInt(1, gId);     //给第一个问号赋值为gId
            pstmt.setString(2, gName);//给第二个问号赋值为gName
            rs = pstmt.executeQuery();
            while (rs.next())
            {
                int gid = rs.getInt("gid");
                String gname = rs.getString(2);//rs结果中第二列的值
    ...
    public ArrayList<SalesMan> querySalesMan(String sName)
    {
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                SalesMan salesMan = new SalesMan(sid,sname,sPassWord);//创建一个SalesMan对象
                SalesManList.add(salesMan); //添加到动态数组中
            }
        } catch (SQLException e)
        {
            e.printStackTrace();
